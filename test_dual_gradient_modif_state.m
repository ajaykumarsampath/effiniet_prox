...
Ptree_dist=factor_apg_eff_dist_state(sys_box_dist,V,Tree);
x=0.1*(S.xmax-S.xmin)+S.xmin;
Nd=size(Tree.stage,1);
Ns=size(Tree.leaves,1);

Y.y=1000*rand(size(sys_box.F{1},1),Nd);

W.x=kron([1;1],Y.y(1:sys_box.nx,:));
W.u=Y.y(sys_box.nx+1:end,:);
%%
...
[Z1,details_apg]=solve_apg_eff_modif_state(sys_box,Tree,Ptree_box,Y,x,opts_apg.state);
[Z1_dist,details_apg_dist]=solve_apg_eff_dist_state(sys_box_dist,Tree,Ptree_dist,W,x,opts_apg.state);
Z.X_APG=Z1.X;
Z.U_APG=Z1.U;
Z.X_APG_dist=Z1.X;
Z.U_APG_dist=Z1.U;
...
dual_grad_yalmip=dual_gradient_modif_state_yalmip(sys_box,Tree,V,grad_opts);
[results,error]=dual_grad_yalmip{{grad_opts.x,grad_opts.u,Y.y}};
...
max(max(abs(Z.U_APG-Z.U_APG_dist)))