...
WtSet1=2*opts_prox.gamma_xbox/(opts_prox.lambda+2);
WtSet2=2*opts_prox.gamma_xs/(opts_prox.lambda+2);
...
while(i<opts_prox.iter)
    ...
    zt=z(:,2);
    z(:,2)=(2*(z(:,1)+y)+gamma*x)/(2+gamma);
    %z(:,2)=(z(:,1)+x+y)/2;
    %z(:,2)=(z(:,1)+x+y)/2;
   
    ProjSet2=max(opts_prox.xs,z(:,2));
    %ProjSet2=min(opts_prox.xmax,ProjSet2);
    
    distance(2)=norm(z(:,2)-ProjSet2,2);
    
    if(distance(2)>WtSet2)
        z(:,2)=zt+WtSet2*(ProjSet2-z(:,2))/distance(2);
    else
        z(:,2)=ProjSet2;
    end
