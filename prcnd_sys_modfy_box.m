...
if(apg_opts.exact_prcnd)
    ...
    for j=1:Nd
        ...
        k=Tree.stage(j)+1;
        sys_new.G{j,1}(nx+1:end,:)=prob(j)*DH_diag((k-1)*nz+1:(k-1)*nz+nu,(k-1)*nz+1:(k-1)*nz+nu)*...
            sys_box.G{j,1}(nx+1:end,:);
        ...
        sys_new.F{j,1}(1:nx,:)=prob(j)*DH_diag((k-1)*nz+nu+1:k*nz,(k-1)*nz+nu+1:k*nz)*...
            sys_box.F{j,1}(1:nx,:);
