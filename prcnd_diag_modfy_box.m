function [ sys_new ] = prcnd_diag_modfy_box( sys,DualHessian,opts)
...
if(sys.cell)
    for j=1:Np
        ...
        sys_new.xmax((j-1)*nx+1:j*nx,1)=DH_diag((j-1)*nz+nu+1:j*nz,...
            (j-1)*nz+nu+1:j*nz)*sys_new.xmax((j-1)*nx+1:j*nx);
        sys_new.xmin((j-1)*nx+1:j*nx,1)=DH_diag((j-1)*nz+nu+1:j*nz,...
            (j-1)*nz+nu+1:j*nz)*sys_new.xmin((j-1)*nx+1:j*nx);
    end
else
    sys_new.F=cell(Np,1);
    sys_new.G=cell(Np,1);
