function [ t ] = proximal_effinet_2(Z,W,sys,opts_prox)
...
if(sys.cell)
    hard_const.x=[W.y(1:2*opts_prox.nx,2:end) W.yt]/opts_prox.lambda;
    hard_const.u=W.y(2*opts_prox.nx+1:end,:)/opts_prox.lambda;
    Nd=size(hard_const.x,2);
    Ns=size(W.yt,2);
    for i=1:Nd
        hard_const.x(:,i)=hard_const.x(:,i)+sys.F{i+1,1}(1:2*sys.nx,:)*Z.X(:,i+1);
        if(i<Nd-Ns+1)
            hard_const.u(:,i)=hard_const.u(:,i)+sys.G{i,1}(2*sys.nx+1:end,:)*Z.U(:,i);
        end
    end 
    ...
    t.u=zeros(2*sys.nu,size(hard_const.u,2));
    t.u=min(opts_prox.umax,hard_const.u(1:sys.nu,:));
    t.u(sys.nu+1:2*sys.nu,:)=min(opts_prox.umin,hard_const.u(sys.nu+1:2*sys.nu,:));
    ...
    hard_const.u=W.y(2*opts_prox.nx+1:end,:)/opts_prox.lambda+sys.G(2*sys.nx+1:end,:)*Z.U;
