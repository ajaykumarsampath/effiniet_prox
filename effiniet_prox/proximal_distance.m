...
% specified.
if(sys_dst.gamma_max==sys_dst.gamma_min)
    prox_details.x=yq;
    prox_details.proj_max=min(yq,sys_dst.xmax);
    prox_details.proj_min=max(yq,sys_dst.xmin);
    
    if((min(yq-sys_dst.xmax<0)==1)&&(min(yq-sys_dst.xmin>0)==1))
        prox_details.x=yq;
    elseif(min(yq-sys_dst.xmax<0)==1)
        if(norm(yq-prox_details.proj_min,2)>sys_dst.gamma_min)
            prox_details.x=yq+sys_dst.gamma_min*...
                (prox_details.proj_min-yq)/(norm(prox_details.proj_min-yq,2));
        else
            prox_details.x=prox_details.proj_min;
        end
    elseif(min(yq-sys_dst.xmin>0)==1)
        if(norm(yq-prox_details.proj_max,2)>sys_dst.gamma_max)
            prox_details.x=yq+sys_dst.gamma_max*...
                (prox_details.proj_max-yq)/(norm(yq-prox_details.proj_max,2));
        else
            prox_details.x=prox_details.proj_max;
        end
    else
        if(norm(yq-prox_details.proj_min,2)>sys_dst.gamma_min)
            yq=yq+sys_dst.gamma_min*(prox_details.proj_min-yq)/(norm(prox_details.proj_min-yq,2));
        else
            yq=prox_details.proj_min;
        end
        pp=min(yq,prox_details.proj_max);
        if(norm(yq-pp,2)>sys_dst.gamma_max)
            prox_details.x=yq+sys_dst.gamma_max*(pp-yq)/(norm(yq-pp,2));
        else
            prox_details.x=pp;
        end
    end
    
    %prox_details.proj_max=min(prox_details.x,sys_dst.xmax);
    %prox_details.proj_min=max(prox_details.x,sys_dst.xmin);
else
    p=[1/2 1/2];
    dim=size(yq,1);
