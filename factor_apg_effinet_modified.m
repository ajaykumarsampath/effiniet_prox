function [ Ptree] = factor_apg_effinet_modified( sys,V,Tree)
...
if(sys.cell)
    ...
    for k=sys.Np:-1:1
        ...
        for j=1:length(nodes_stage)
            ...
            Ptree.Phi{nodes_stage(j),1}=Ptree.omega{nodes_stage(j),1}*Ptree.Gbar{nodes_stage(j),1}';
            ...
            Ptree.d{nodes_stage(j),1}=Ptree.K{nodes_stage(j),1}'*Ptree.Gbar{nodes_stage(j),1}';
   ...
   for k=sys.Np:-1:1
        ...
        for j=1:length(nodes_stage)
            ...
            Ptree.omega{nodes_stage(j),1}=-0.5*(Wv_k\eye(nv));
            Ptree.Phi{nodes_stage(j),1}=Ptree.omega{nodes_stage(j),1}*Gbar';
            Ptree.Theta{nodes_stage(j),1}=Ptree.omega{nodes_stage(j),1}*Bbar';
            Ptree.K{nodes_stage(j),1}=-2*Tree.prob(nodes_stage(j))*...
                (Ptree.omega{nodes_stage(j),1}*Wv);
            Ptree.d{nodes_stage(j),1}=Ptree.K{nodes_stage(j),1}'*Gbar';
            Ptree.f{nodes_stage(j),1}=Ptree.K{nodes_stage(j),1}'*Bbar';
        end
        if(k==1)
            Ptree.P{1}=-Wv*Ptree.K{1};
