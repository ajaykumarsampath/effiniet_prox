...
for k=sys_box.Np:-1:1
    ...
    if(k==1)
        ...
        v(:,1)=Ptree_box.Phi{1}*Y.x(:,1)+Ptree_box.Psi{1}*Y.u(:,1)+...
            Ptree_box.Theta{1}*q(:,2)+Ptree_box.omega{1}*sigma(:,1);
        
        r(:,1)=Ptree_box.K{1}*sigma(:,1)+Ptree_box.d{1}*Y.x(:,1)+...
            Ptree_box.f{1}*Y.u(:,1)+Ptree_box.g{1}*q(:,2);
        q(:,1)=sys_box.A'*sys_box.F{1}'*Y.x(:,1)+sys_box.A'*q(:,2);
    else
        Pnodes_stage=find(Tree.stage==k-2);
        for j=1:length(nodes_stage)
            sigma(:,nodes_stage(j))=r(:,nodes_stage(j)+1)+opts.beta(:,nodes_stage(j));
            v(:,nodes_stage(j))=Ptree_box.Phi{nodes_stage(j)}*Y.x(:,nodes_stage(j))+...
                Ptree_box.Psi{nodes_stage(j)}*Y.u(:,nodes_stage(j))+...
            ...
            for l=1:length(nchild)
                if(l==1)
                    r(:,Pnodes_stage(j)+1)=Ptree_box.K{nchild(l)}*sigma(:,nchild(l))+...
                        Ptree_box.d{nchild(l)}*Y.x(:,nchild(l))+...
                        Ptree_box.f{nchild(l)}*Y.u(:,nchild(l))+...
                        Ptree_box.g{nchild(l)}*q(:,nchild(l)+1);
                    q(:,Pnodes_stage(j)+1)=sys_box.A'*sys_box.F{nchild(l)}'*Y.x(:,nchild(l))...
                        +sys_box.A'*q(:,nchild(l)+1);
                else
                    r(:,Pnodes_stage(j)+1)=Ptree_box.K{nchild(l)}*sigma(:,nchild(l))+...
                        Ptree_box.d{nchild(l)}*Y.x(:,nchild(l))+...
                        Ptree_box.f{nchild(l)}*Y.u(:,nchild(l))+...
                        Ptree_box.g{nchild(l)}*q(:,nchild(l)+1)+r(:,Pnodes_stage(j)+1);
                    q(:,Pnodes_stage(j)+1)=sys_box.A'*sys_box.F{nchild(l)}'*Y.x(:,nchild(l))...
