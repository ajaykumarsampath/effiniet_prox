function [ zoptimiser ] = effinet_modifi_distance_yalmip( sys,Tree,V,opts)
%  
...
for i=1:Nd-Ns+1
    if(i==1)
        ...
        const=const+(sys.xs(1:sys.nx,1)<=Xs(:,2));
    else
        ...
        for l=1:length(nchild)
            Jobj=Jobj+Tree.prob(nchild(l),1)*(V.alpha(stage+1,:)*U(:,nchild(l))+(U(:,nchild(l))-...
                ...
                +sys.gamma_xs*norm(X(:,nchild(l))-Xs(:,nchild(l)));
